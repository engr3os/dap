...
if __name__ == '__main__':
	...
	X_tr = np.load('../brain4cars_data/npy/X_train.npy')
	Y_tr = np.load('../brain4cars_data/npy/Y_train.npy')
	X_te = np.load('../brain4cars_data/npy/X_test.npy')
	Y_te = np.load('../brain4cars_data/npy/Y_test.npy')
	...
	model_name = "BiRNN_GRU_model_brain4cars.ckpt"
	...
	target_names = ['lturn','rturn','lchange','rchange','straight'] 
	label_nums = {'lturn':0, 'rturn':1, 'lchange':2, 'rchange':3, 'straight':4}
	#-----------------------------------------------------------------------------------------------------------------------------#
	...
	if not load_model:
		with tf.Session() as sess:
			...
			while step*train_batch_size<training_iters and acc <0.99: #step * batch_size < training_iters:
	...
	np.savetxt("gt_labels.txt",Y_test,fmt='%i')
	print "Raw Pred Labels[0]: ",raw_p_labels_final[0]
	...
	def showValues(metric,n_steps):
		a = np.arange(-n_steps,0)+1
		for x,y in zip(a, metric):                                       
			plt.annotate('%.2f' %y, xy=(x,y), textcoords='data')

	def plotData(dictmap,label_nums,marker_label_nums,label,n_steps,marker):
		for key in label_nums:
			plt.plot((np.arange(-n_steps,0)+1),dictmap[key],color=marker_label_nums[key],marker=marker,label= label+key)
		plt.grid(True)
		plt.axis([-n_steps, 0, 0.5, 1.1])
		plt.xlabel('Time to Manuever (x0.8 sec)')
		plt.ylabel('Value')
		#plt.legend(bbox_to_anchor=(0.6, 1), loc=2, borderaxespad=0.)
		plt.legend(loc="upper right")

	def plotDataClubbed(precision, recall, f1, n_steps, label):
		plt.plot((np.arange(-n_steps,0)+1),precision,color='b',marker='x',label= 'Precision_'+label)
		showValues(precision,n_steps)
		plt.plot((np.arange(-n_steps,0)+1),recall,color='r',marker='o',label= 'Recall_'+label)
		showValues(recall,n_steps)
		plt.plot((np.arange(-n_steps,0)+1),f1,color='k',marker='+',label= 'F1_'+label)
		showValues(f1,n_steps)
		plt.grid(True)
		plt.axis([-n_steps, 0, 0.5, 1.1])
		plt.xlabel('Time to Manuever (x0.8 sec)')
		plt.ylabel('Value')
	...
	precision,recall,f1 = {},{},{}
	for key in label_nums:
		precision[key],recall[key],f1[key]= [],[],[]
	for i in range(n_steps):
		tmpP = precision_score(Y_test[:,i],p_labels_final[:,i],average=None)
		tmpR = recall_score(Y_test[:,i],p_labels_final[:,i],average=None)
		tmpF = f1_score(Y_test[:,i],p_labels_final[:,i],average=None)
		for key in label_nums:
			precision[key].append(tmpP[label_nums[key]])
			recall[key].append(tmpR[label_nums[key]])
			f1[key].append(tmpF[label_nums[key]])


	precision_lane_change = [np.mean([precision[key][i] for key in ["lchange","rchange"]]) for i in range(n_steps)]
	recall_lane_change = [np.mean([recall[key][i] for key in ["lchange","rchange"]]) for i in range(n_steps)]
	f1_lane_change = [np.mean([f1[key][i] for key in ["lchange","rchange"]]) for i in range(n_steps)]
	precision_turn = [np.mean([precision[key][i] for key in ["lturn","rturn"]]) for i in range(n_steps)]
	recall_turn = [np.mean([recall[key][i] for key in ["lturn","rturn"]]) for i in range(n_steps)]
	f1_turn = [np.mean([f1[key][i] for key in ["lturn","rturn"]]) for i in range(n_steps)]
	precision_all = [np.mean([precision[key][i] for key in ["lchange","rchange","lturn","rturn"]]) for i in range(n_steps)]
	recall_all = [np.mean([recall[key][i] for key in ["lchange","rchange","lturn","rturn"]]) for i in range(n_steps)]
	f1_all = [np.mean([f1[key][i] for key in ["lchange","rchange","lturn","rturn"]]) for i in range(n_steps)]


	marker_label_nums = {'lturn':'b', 'rturn':'r', 'lchange':'k', 'rchange':'g', 'straight':'c'}

	fig1 = plt.figure(1)
	plt.plot((np.arange(-n_steps,0)+1), col_acc, '-ro')
	plt.xlabel('Time to Maneuver (x0.8 sec)')
	plt.ylabel('Accuracy')
	plt.title('Piecewise Accuracy')
	a = np.arange(-n_steps,0)+1
	showValues(col_acc,n_steps)
	plt.axis([-n_steps, 0, 0.5, 1])

	fig2 = plt.figure(2)
	plotData(precision,label_nums,marker_label_nums,"Precision_",n_steps,'x')
	plt.title("Classwise precision vs time to maneuver")
	

	fig3 = plt.figure(3)
	plotData(recall,label_nums,marker_label_nums,"Recall_",n_steps,'o')
	plt.title("Classwise recall va time to maneuver")
	

	fig4 = plt.figure(4)
	plotData(f1,label_nums,marker_label_nums,"F1_",n_steps,'+')
	plt.title("Classwise F1 score vs time to maneuver")

	fig5 = plt.figure(5)
	plotDataClubbed(precision_lane_change, recall_lane_change,f1_lane_change,n_steps,"lane_change")
	plt.title("Precision, Recall and F1 score for lane change")

	fig6 = plt.figure(6)
	plotDataClubbed(precision_turn,recall_turn,f1_turn,n_steps,"turn")
	plt.title("Precision, Recall and F1 score for turn")

	fig7 = plt.figure(7)
	plotDataClubbed(precision_all,recall_all,f1_all,n_steps,"all")
	plt.title("Precision, Recall and F1 score for all")
